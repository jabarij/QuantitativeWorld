...
namespace QuantitativeWorld.Tests.Formatting
{
    partial class WeightFormatterTests
    {
        public class StringFormat : WeightFormatterTests
        {
            ...
            [InlineData(123.456d, "Weight is: {0:" + StandardFormats.SI + "}", "pl-PL", "Weight is: 123,456 kg", "weight in standard format should be printed with double separator appropriate to given culture")]
            public void StringFormat_StandardFormat_ShouldReturnProperValue(double kilograms, string format, string cultureName, string expectedResult, string reason)
            {
                // arrange
            ...
            public void StringFormat_CustomFormat_ShouldReturnProperValue(double kilograms, string customFormat, string cultureName, string expectedResult)
            {
                // arrange
                var formatter = new WeightFormatter(CultureInfo.GetCultureInfo(cultureName));
                var weight = new Weight(kilograms, WeightUnit.Kilogram);
