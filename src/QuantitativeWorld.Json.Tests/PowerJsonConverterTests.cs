...
namespace QuantitativeWorld.Json.Tests
{
    ...
    public class PowerJsonConverterTests : TestsBase
    {
        ...
        {
            // arrange
            var converter = new PowerJsonConverter(testData.Format);
            var unitConverter = new PowerUnitJsonConverter(testData.UnitFormat);
        ...
        private static IEnumerable<PowerSerializeTestData> GetSerializeTestData()
        {
            ...
            yield return new PowerSerializeTestData(
                ...
                expectedJsonPattern:
                "{\"Watts\":5000(\\.0+)?,\"Unit\":{\"Name\":\"kilowatt\",\"Abbreviation\":\"kW\",\"ValueInWatts\":1000(\\.0+)?}}");
            yield return new PowerSerializeTestData(
                value: testValue,
                format: PowerJsonSerializationFormat.AsValueWithUnit,
                unitFormat: LinearUnitJsonSerializationFormat.AlwaysFull,
                expectedJsonPattern:
                "{\"Value\":5(\\.0+)?,\"Unit\":{\"Name\":\"kilowatt\",\"Abbreviation\":\"kW\",\"ValueInWatts\":1000(\\.0+)?}}");
        }
  ...
  ""Watts"": 123.456
            ...
            // act
            var result = Deserialize<Power>(json, converter);
  ...
  ""Unit"": ""kW""
            ...
            var converter = new PowerJsonConverter();
            var unitConverter = new PowerUnitJsonConverter();
            var expectedResult =
                new Power(watts: (number)123.456m)
                    .Convert(PowerUnit.Kilowatt);

            // act
            var result = Deserialize<Power>(json, converter, unitConverter);
  ...
  ""Unit"": {
    ...
    ""ValueInWatts"": 1000
        ...
        public void SerializeAndDeserializeWithWatts_ShouldBeIdempotent(PowerJsonSerializationFormat format,
            LinearUnitJsonSerializationFormat unitFormat)
        {
            // arrange
            var power = Fixture.Create<Power>();
            var converter = new PowerJsonConverter(format);
            var unitConverter = new PowerUnitJsonConverter(unitFormat);

            // act
            string serializedPower1 = Serialize(power, converter, unitConverter);
            var deserializedPower1 = Deserialize<Power>(serializedPower1, converter, unitConverter);
            string serializedPower2 = Serialize(deserializedPower1, converter, unitConverter);
            var deserializedPower2 = Deserialize<Power>(serializedPower2, converter, unitConverter);
